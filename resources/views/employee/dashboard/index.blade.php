...
@section('content')
      ...
      <div class="panel">
        ...
        <div style="padding:16px 22px;border-top:1px solid var(--border)">
          ...
          <div style="display:grid;grid-template-columns:repeat(7,1fr);text-align:center;gap:2px">
            ...
            @endphp
            @foreach($days as $d)
                <div style='font-size:10px;font-weight:600;color:var(--text-3);padding:3px 0'>{{ $d }}</div>
            @endforeach
            ...
            @for($d=1; $d<=$totalDays; $d++)
              @php
                ...
                $isLogDone = in_array($d,$logDays);
              @endphp
              <div style='{{ $s }}'>
                {{ $d }}
                @if($isLogDone)
                    <span style='position:absolute;bottom:2px;left:50%;transform:translateX(-50%);width:3px;height:3px;border-radius:50%;background:{{ $d==$today ? '#fff' : 'var(--accent)' }}'></span>
                @endif
              </div>
            @endfor
          </div>
    ...
    <div class="bottom-row">
      ...
      <div class="panel">
        ...
        <div class="log-body">
          <div class="log-date">📅 {{ date('l, d F Y') }} — editable until midnight</div>
          <div class="log-sec">
          ...
          <button class="log-save" id="log-btn" onclick="window.location.href='{{ url('/employee/dailylogs') }}'">Manage All Logs</button>
